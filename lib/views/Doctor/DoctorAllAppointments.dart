...
class _DoctorAllAppointmentsState extends State<DoctorAllAppointments> {
  ...
  fetchPastAppointments() async{
    final response = await get(Uri.parse("$SERVER_ADDRESS/api/doctoruappointment?doctor_id=$userId"));
    print(Uri.parse("$SERVER_ADDRESS/api/doctoruappointment?doctor_id=$userId"));
  ...
  loadmore() async{
    if(nextUrl != "null") {
      ...
      print(Uri.parse("$nextUrl&doctor_id=$userId"));
      if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        if (jsonResponse['success'] == "1") {
          setState(() {
