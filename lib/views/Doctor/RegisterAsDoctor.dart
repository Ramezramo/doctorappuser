...
import 'package:doctorapplast/databaseXoperations/WriteOnDataBase.dart';
...
MakeChangesDBM dbm = MakeChangesDBM();
  ...
  registerUser() async {
    if (name.isEmpty) {
      ...
      try {
        ...
        dbm.prepareDataBaseEnvironment(
            "doctorsloggedin", {"name":name,"phone":phoneNumber,"email":email,"piclink":webpiclink,"password":password});
        Navigator.pop(context);
        Navigator.pop(context);
      } catch (e) {
        if (e is FirebaseAuthException) {
          if (e.code == 'email-already-in-use') {
            ...
            // Navigator.pop(context);
            messageDialog("this email is already signed in try log in", "");
          ...
          child: Column(
            children: [
              ...
              TextField(
                keyboardType: TextInputType.emailAddress,
                decoration: InputDecoration(
                    labelText: "Please paste a temporary web profile photo link",
                    labelStyle: GoogleFonts.poppins(
                        color: LIGHT_GREY_TEXT, fontWeight: FontWeight.w400),
                    errorText: ispicError ? "please paste a link" : null,
                    border: UnderlineInputBorder(),
                    focusedBorder: UnderlineInputBorder(
                        borderSide: BorderSide(color: LIGHT_GREY_TEXT))),
                style: GoogleFonts.poppins(
